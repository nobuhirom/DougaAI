...
| `douga ui` | 制作画面を開く（ボード / ネタ / プロジェクト / キャラクター / 環境） |
...
制作画面も同じ方針で、**見る・選ぶ・直す・焼く**を担う。画面から AI は呼ばない。
音声生成とレンダリングは機械の工程なので画面から実行できる。

- **ボード**: 12工程を5段階（企画 / 台本 / 制作 / 展開 / 投稿）の列にして、全プロジェクトをカードで並べる。複数を別々の段階で並行して進められる
- **プロジェクト**: 1本の12工程。成果物の編集（修正はログに残る）、音声生成・レンダリングの実行、動画の再生
- 未実装の工程も隠さず「未実装・フェーズN」と出す
