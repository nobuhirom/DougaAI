...
台本・サムネイル・ショートの手順書は、プロジェクトのテーマ（`meta.theme`）にある
`themes/<id>/style.md` を末尾に足して使う。同じ手順書でも、企業向けなら誇張を避け
ナレーション1人、解説向けなら掛け合い、といった違いをここで与える。
配色・形式・BGM の既定は `themes/<id>/theme.json`。
